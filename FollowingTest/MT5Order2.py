...
def login_mt5():
    ...
    path="D:/Program Files/MetaTrader5_2/terminal64.exe"
    ...
    print("start to read from the global val")
    if(GlobalVariableCheck("FollowSignal_Symbol")){
        symbol = GlobalVariableGet("FollowSignal_Symbol");
        action = (ENUM_ORDER_TYPE)GlobalVariableGet("FollowSignal_Type");
        price = GlobalVariableGet("FollowSignal_Price");
        volume = GlobalVariableGet("FollowSignal_Volume");
        time = (datetime)GlobalVariableGet("FollowSignal_Time");
        print("start to follow order at: {time}")
        // 执行跟单操作...
        create_order(symbol, action, volume,price)
        // 清除信号
        GlobalVariableDel("FollowSignal_Symbol");
        GlobalVariableDel("FollowSignal_Type");
        GlobalVariableDel("FollowSignal_Price");
        GlobalVariableDel("FollowSignal_Volume");
        GlobalVariableDel("FollowSignal_Time");
    }
    return True
...
# 5. 下单交易
# ================================
def create_order(symbol, action, volume, price, sl=None, tp=None):
      if(GlobalVariableCheck("FollowSignal_Symbol"))
        {
            symbol = GlobalVariableGet("FollowSignal_Symbol");
            ENUM_ORDER_TYPE type = (ENUM_ORDER_TYPE)GlobalVariableGet("FollowSignal_Type");
            double price = GlobalVariableGet("FollowSignal_Price");
            datetime time = (datetime)GlobalVariableGet("FollowSignal_Time");
...
def main_menu():
    while True:
        ...
        if choice == "1":
            ...
            create_order(symbol, action, volume,None)
        elif choice == "6":
